...
def read_line_file(
    ...
    """
    Read one event-camera recording plus one Hough-style CSV and return aligned frames
    for a single regression target such as ``slope`` or ``intercept``.
    """

    print(f"\n{'='*70}")
    print("Starting data loading for line-regression experiment...")
    ...
    if timestamp_column not in df.columns:
        raise KeyError(
            f"CSV file '{CSV_PATH}' is missing required timestamp column '{timestamp_column}'."
        )
    if label_column not in df.columns:
        raise KeyError(
            f"CSV file '{CSV_PATH}' is missing required target column '{label_column}'."
        )

    timestamps = df[timestamp_column].values
    data_labels = df[label_column].values
