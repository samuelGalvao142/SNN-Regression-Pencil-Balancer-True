...
x_state2 = torch.load(WEIGHTS_PATH, map_location=DEVICE)
xNetwork2.load_state_dict(x_state2)
xNetwork2.to(DEVICE)
xNetwork2.eval()
...
y_state2 = torch.load(WEIGHTS_PATH, map_location=DEVICE)
yNetwork2.load_state_dict(y_state2)
yNetwork2.to(DEVICE)
yNetwork2.eval()

functional.reset_net(xNetwork)
functional.reset_net(yNetwork)
functional.reset_net(xNetwork2)
functional.reset_net(yNetwork2)
...
while camera.isRunning():
    ...
    with torch.no_grad():
        ...
        xOutput2 = xNetwork2(events)
        yOutput2 = yNetwork2(events)
