...
template <typename T>
std::enable_if_t<std::is_same_v<T, bfloat16>, T> rsqrt(T x) {
  return sycl::ext::oneapi::bfloat16{sycl::rsqrt(float{x})};
}

// TODO: If N is divisible by 2, we can call rsqrt for two packed values instead
// of scalar on NVIDIA and pISA targets.
template <size_t N>
sycl::marray<bfloat16, N> rsqrt(sycl::marray<bfloat16, N> x) {
  sycl::marray<bfloat16, N> res;
  for (size_t i = 0; i < N; i++) {
    res[i] = rsqrt(x[i]);
  }
...
rsqrt(T x) {
  ...
  auto Res = rsqrt(FVec);
  return Res.template convert<bfloat16>();
#else  // defined(__SYCL_DEVICE_ONLY__) && (defined(__SPIR__) ||
       // defined(__SPIRV__))
  sycl::vec<bfloat16, N> res;
  for (size_t i = 0; i < N; i++) {
    res[i] = rsqrt(x[i]);
