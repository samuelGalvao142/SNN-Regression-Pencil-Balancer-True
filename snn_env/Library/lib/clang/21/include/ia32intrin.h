...
/// Adds the value of R2 to R1, then performs a rotate-through-carry
/// right operation on the result.
///
/// \headerfile <x86intrin.h>
///
/// \code
/// unsigned int _add_rcr(unsigned int R1, unsigned int R2);
/// \endcode
///
/// \param R1
///    The first operand to be added.
/// \param R2
///    The number to be added to R1.
/// \returns The result of addition followed by a rotate-through-carry right
/// operation.
///
static unsigned int __DEFAULT_FN_ATTRS _add_rcr(unsigned int R1,
                                                unsigned int R2) {
  __asm__ __volatile__("add %1, %0\nrcr $1, %0" : "+r"(R1) : "r"(R2) : "cc");
  return R1;
}
#endif
